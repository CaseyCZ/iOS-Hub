...
const GUIDE_COPY = {
  en: {
    ...
    sideInstallerTitle:'Install SideStore without a PC on iOS 27', sideInstallerDesc:'SideInstaller installs SideStore or SideStore + LiveContainer fully on-device on iOS/iPadOS 27. Its current official requirements list iOS 27; Wi-Fi is required for the first-time pairing step.', sideInstallerSafety:'Use only sideinstaller.net or the official FrizzleM/SideInstaller GitHub repository when entering Apple account credentials.', sideInstallerOpen:'Open official SideInstaller ↗',
    sideInstallerBest:'On-device SideStore / LiveContainer setup', sideInstallerDevice:'iOS/iPadOS 27', sideInstallerComputer:'No PC on iOS/iPadOS 27; Wi-Fi is required for first-time pairing', sideInstallerRefresh:'Installer only; refreshing is then handled by SideStore',
...
reportHelpTitle:'Still not working?', reportHelpDesc:'Open a pre-filled GitHub report with the selected problem so it is easier to diagnose.', reportHelpButton:'Report this problem ↗'
  ...
  cs: {
    ...
    sideInstallerTitle:'SideStore bez PC na iOS 27', sideInstallerDesc:'SideInstaller umí na iOS/iPadOS 27 nainstalovat SideStore nebo SideStore + LiveContainer přímo v zařízení. Aktuální oficiální požadavky uvádějí iOS 27; Wi-Fi je potřeba pro první pairing.', sideInstallerSafety:'Při zadávání Apple účtu používej pouze sideinstaller.net nebo oficiální GitHub FrizzleM/SideInstaller.', sideInstallerOpen:'Otevřít oficiální SideInstaller ↗',
    sideInstallerBest:'Instalace SideStore / LiveContainer přímo v zařízení', sideInstallerDevice:'iOS/iPadOS 27', sideInstallerComputer:'Na iOS/iPadOS 27 bez PC; Wi-Fi je potřeba pro první pairing', sideInstallerRefresh:'Je to instalátor; další obnovování řeší SideStore',
    ...
    troubleLoginQ:'Selže přihlášení Apple ID, HTTP 503 nebo 2FA', troubleLoginA:'U problémů s přihlášením v SideStore nejdřív změň Anisette Server. Pokud nepřijde ověřovací kód Apple, vyžádej si ho ručně v nastavení Apple účtu. Při dočasných chybách Apple služeb nezkoušej přihlášení opakovaně v rychlém sledu.',
...
reportHelpTitle:'Pořád to nejde?', reportHelpDesc:'Otevři předvyplněné hlášení na GitHubu s vybraným problémem, aby šla chyba rychleji dohledat.', reportHelpButton:'Nahlásit tento problém ↗'
  ...
  de: {
    ...
    sideInstallerTitle:'SideStore ohne PC auf iOS 27 installieren', sideInstallerDesc:'SideInstaller installiert SideStore oder SideStore + LiveContainer unter iOS/iPadOS 27 vollständig auf dem Gerät. Die aktuellen offiziellen Anforderungen nennen iOS 27; Wi-Fi wird für das erste Pairing benötigt.', sideInstallerSafety:'Bei der Eingabe deines Apple-Accounts nur sideinstaller.net oder das offizielle GitHub-Repository FrizzleM/SideInstaller verwenden.', sideInstallerOpen:'Offiziellen SideInstaller öffnen ↗',
    sideInstallerBest:'SideStore / LiveContainer direkt auf dem Gerät einrichten', sideInstallerDevice:'iOS/iPadOS 27', sideInstallerComputer:'Unter iOS/iPadOS 27 kein PC; Wi-Fi wird für das erste Pairing benötigt', sideInstallerRefresh:'Nur Installer; die Aktualisierung übernimmt danach SideStore',
...
reportHelpTitle:'Funktioniert es immer noch nicht?', reportHelpDesc:'Öffne einen vorausgefüllten GitHub-Bericht mit dem ausgewählten Problem.', reportHelpButton:'Problem melden ↗'
  ...
  es: {
    ...
    sideInstallerTitle:'Instalar SideStore sin PC en iOS 27', sideInstallerDesc:'SideInstaller instala SideStore o SideStore + LiveContainer completamente desde el dispositivo en iOS/iPadOS 27. Los requisitos oficiales actuales indican iOS 27; se necesita Wi-Fi para el primer emparejamiento.', sideInstallerSafety:'Al introducir tu cuenta Apple usa solo sideinstaller.net o el repositorio oficial FrizzleM/SideInstaller en GitHub.', sideInstallerOpen:'Abrir SideInstaller oficial ↗',
    sideInstallerBest:'Configurar SideStore / LiveContainer desde el dispositivo', sideInstallerDevice:'iOS/iPadOS 27', sideInstallerComputer:'Sin PC en iOS/iPadOS 27; se necesita Wi-Fi para el primer emparejamiento', sideInstallerRefresh:'Solo instala; después la renovación la gestiona SideStore',
    ...
    troubleLoginQ:'Falla el acceso con Apple ID, HTTP 503 o 2FA', troubleLoginA:'Para problemas de acceso en SideStore, prueba primero otro servidor Anisette. Si no llega el código de verificación de Apple, solicítalo manualmente en los ajustes de la cuenta Apple. Evita intentos rápidos y repetidos si los servicios de Apple están devolviendo errores temporales.',
...
reportHelpTitle:'¿Sigue sin funcionar?', reportHelpDesc:'Abre un informe de GitHub pre-rellenado con el problema seleccionado.', reportHelpButton:'Reportar este problema ↗'
  ...
  fr: {
    ...
    sideInstallerTitle:'Installer SideStore sans PC sur iOS 27', sideInstallerDesc:'SideInstaller installe SideStore ou SideStore + LiveContainer entièrement sur l’appareil sous iOS/iPadOS 27. Les exigences officielles actuelles indiquent iOS 27 ; le Wi-Fi est requis pour le premier appairage.', sideInstallerSafety:'Pour saisir votre compte Apple, utilisez uniquement sideinstaller.net ou le dépôt GitHub officiel FrizzleM/SideInstaller.', sideInstallerOpen:'Ouvrir SideInstaller officiel ↗',
    sideInstallerBest:'Configurer SideStore / LiveContainer sur l’appareil', sideInstallerDevice:'iOS/iPadOS 27', sideInstallerComputer:'Pas de PC sous iOS/iPadOS 27 ; le Wi-Fi est requis pour le premier appairage', sideInstallerRefresh:'Installateur uniquement ; l’actualisation est ensuite gérée par SideStore',
...
const SETUP_RESULTS = {
  en: {
    ...
    pairingNeeded:{title:'iOS 17–26: a computer is required for the standard SideStore setup',text:'The current official SideInstaller requirements list iOS 27. On iOS 17–26, use a supported SideStore setup with a computer for the initial pairing instead of relying on SideInstaller.',steps:['Use the current SideStore installation instructions.','Create the required pairing data with a supported computer-based setup.','After setup, use SideStore and LocalDevVPN on the device.'],primary:'Návod k instalaci SideStore ↗',url:'https://docs.sidestore.io/docs/installation/prerequisites',secondary:'SideStore pairing guide ↗',secondaryUrl:'https://docs.sidestore.io/docs/advanced/pairing-file'},
    checkVersionNoPc:{title:'Check your iOS version first',text:'The official SideInstaller no-PC path currently requires iOS 27. If the device is on another version, use the supported SideStore installation method for that version, which can require a computer.',steps:['Open Settings → General → About.','Check the exact iOS/iPadOS version.','Choose iOS 27 or iOS 17–26 in this wizard.'],primary:'Návod k instalaci SideStore ↗',url:'https://docs.sidestore.io/docs/installation/prerequisites'},
  ...
  cs: {
    ...
    pairingNeeded:{title:'iOS 17–26: pro standardní nastavení SideStore je potřeba počítač',text:'Aktuální oficiální požadavky SideInstalleru uvádějí iOS 27. Na iOS 17–26 použij pro první pairing podporované nastavení SideStore přes počítač a nespoléhej na SideInstaller.',steps:['Použij aktuální instalační návod SideStore.','Vytvoř potřebná pairing data podporovaným postupem přes počítač.','Po nastavení používej SideStore a LocalDevVPN přímo v zařízení.'],primary:'Návod k instalaci SideStore ↗',url:'https://docs.sidestore.io/docs/installation/prerequisites',secondary:'Návod k pairingu SideStore ↗',secondaryUrl:'https://docs.sidestore.io/docs/advanced/pairing-file'},
    checkVersionNoPc:{title:'Nejdřív zjisti přesnou verzi iOS',text:'Oficiální cesta SideInstalleru bez PC aktuálně vyžaduje iOS 27. Na jiné verzi použij podporovaný instalační postup SideStore pro danou verzi; ten může vyžadovat počítač.',steps:['Otevři Nastavení → Obecné → Informace.','Zjisti přesnou verzi iOS/iPadOS.','V průvodci pak vyber iOS 27 nebo iOS 17–26.'],primary:'Návod k instalaci SideStore ↗',url:'https://docs.sidestore.io/docs/installation/prerequisites'},
  ...
  de: {
    ...
    pairingNeeded:{title:'iOS 17–26: Für das normale SideStore-Setup ist ein Computer erforderlich',text:'Die aktuellen offiziellen SideInstaller-Anforderungen nennen iOS 27. Unter iOS 17–26 nutze für das erste Pairing ein unterstütztes SideStore-Setup mit Computer und verlasse dich nicht auf SideInstaller.',steps:['Die aktuelle SideStore-Installationsanleitung verwenden.','Die erforderlichen Pairing-Daten mit einem unterstützten computerbasierten Setup erstellen.','Nach der Einrichtung SideStore und LocalDevVPN auf dem Gerät verwenden.'],primary:'SideStore-Setup-Anleitung ↗',url:'https://docs.sidestore.io/docs/installation/prerequisites',secondary:'SideStore Pairing-Anleitung ↗',secondaryUrl:'https://docs.sidestore.io/docs/advanced/pairing-file'},
    checkVersionNoPc:{title:'Zuerst die genaue iOS-Version prüfen',text:'Der offizielle SideInstaller-Weg ohne PC erfordert derzeit iOS 27. Auf anderen Versionen die unterstützte SideStore-Installationsmethode für diese Version verwenden; sie kann einen Computer erfordern.',steps:['Einstellungen → Allgemein → Info öffnen.','Genaue iOS/iPadOS-Version prüfen.','Danach im Assistenten iOS 27 oder iOS 17–26 auswählen.'],primary:'SideStore-Setup-Anleitung ↗',url:'https://docs.sidestore.io/docs/installation/prerequisites'},
  ...
  es: {
    ...
    pairingNeeded:{title:'iOS 17–26: se necesita un ordenador para la configuración estándar de SideStore',text:'Los requisitos oficiales actuales de SideInstaller indican iOS 27. En iOS 17–26 usa una configuración compatible de SideStore con ordenador para el primer pairing y no dependas de SideInstaller.',steps:['Usa las instrucciones actuales de instalación de SideStore.','Crea los datos de pairing necesarios con una configuración compatible basada en ordenador.','Después de la configuración, usa SideStore y LocalDevVPN en el dispositivo.'],primary:'Guía de configuración de SideStore ↗',url:'https://docs.sidestore.io/docs/installation/prerequisites',secondary:'Guía de pairing SideStore ↗',secondaryUrl:'https://docs.sidestore.io/docs/advanced/pairing-file'},
    checkVersionNoPc:{title:'Comprueba primero tu versión exacta de iOS',text:'La ruta sin PC depende de la versión exacta. En iOS 27, SideInstaller funciona completamente en el dispositivo. En iOS 17–26, SideInstaller v1 solo funciona si ya existe un archivo de pairing.',steps:['Abre Ajustes → General → Información.','Comprueba la versión exacta de iOS/iPadOS.','Después elige iOS 27 o iOS 17–26 en este asistente.'],primary:'Guía de configuración de SideStore ↗',url:'https://docs.sidestore.io/docs/installation/prerequisites'},
  ...
  fr: {
    ...
    pairingNeeded:{title:'iOS 17–26 : un ordinateur est requis pour la configuration SideStore standard',text:'Les exigences officielles actuelles de SideInstaller indiquent iOS 27. Sous iOS 17–26, utilisez une configuration SideStore prise en charge avec ordinateur pour le premier appairage au lieu de compter sur SideInstaller.',steps:['Utiliser les instructions d’installation SideStore actuelles.','Créer les données d’appairage requises avec une configuration prise en charge sur ordinateur.','Après la configuration, utiliser SideStore et LocalDevVPN sur l’appareil.'],primary:'Guide de configuration SideStore ↗',url:'https://docs.sidestore.io/docs/installation/prerequisites',secondary:'Guide d’appairage SideStore ↗',secondaryUrl:'https://docs.sidestore.io/docs/advanced/pairing-file'},
    checkVersionNoPc:{title:'Vérifiez d’abord la version exacte d’iOS',text:'Le parcours officiel SideInstaller sans PC nécessite actuellement iOS 27. Sur les autres versions, utilisez la méthode d’installation SideStore prise en charge pour cette version ; elle peut nécessiter un ordinateur.',steps:['Ouvrir Réglages → Général → Informations.','Vérifier la version exacte d’iOS/iPadOS.','Choisir ensuite iOS 27 ou iOS 17–26 dans cet assistant.'],primary:'Guide de configuration SideStore ↗',url:'https://docs.sidestore.io/docs/installation/prerequisites'},
