...
const GUIDE_COPY = {
  en: {
    ...
    sideInstallerTitle:'Install SideStore without a PC on iOS 27', sideInstallerDesc:'SideInstaller installs SideStore or SideStore + LiveContainer fully on-device on iOS/iPadOS 27. The current official requirements list iOS 27, Wi-Fi for first-time setup and no PC requirement.', sideInstallerSafety:'Use only sideinstaller.net or the official FrizzleM/SideInstaller GitHub repository when entering Apple account credentials.', sideInstallerOpen:'Open official SideInstaller ↗',
    sideInstallerBest:'On-device SideStore / LiveContainer setup', sideInstallerDevice:'iOS/iPadOS 27', sideInstallerComputer:'No PC on iOS/iPadOS 27; Wi-Fi is required for first-time setup', sideInstallerRefresh:'Installer only; refreshing is then handled by SideStore',
...
reportHelpTitle:'Still not working?', reportHelpDesc:'Open a pre-filled GitHub report with the selected problem so it is easier to diagnose.', reportHelpButton:'Report this problem ↗'
  ...
  cs: {
    ...
    sideInstallerTitle:'SideStore bez PC na iOS 27', sideInstallerDesc:'SideInstaller umí na iOS/iPadOS 27 nainstalovat SideStore nebo SideStore + LiveContainer přímo v zařízení. Aktuální oficiální požadavky uvádějí iOS 27, Wi-Fi pro první nastavení a bez potřeby PC.', sideInstallerSafety:'Při zadávání Apple účtu používej pouze sideinstaller.net nebo oficiální GitHub FrizzleM/SideInstaller.', sideInstallerOpen:'Otevřít oficiální SideInstaller ↗',
    sideInstallerBest:'Instalace SideStore / LiveContainer přímo v zařízení', sideInstallerDevice:'iOS/iPadOS 27', sideInstallerComputer:'Na iOS/iPadOS 27 bez PC; pro první nastavení je potřeba Wi-Fi', sideInstallerRefresh:'Je to instalátor; další obnovování řeší SideStore',
...
reportHelpTitle:'Pořád to nejde?', reportHelpDesc:'Otevři předvyplněné hlášení na GitHubu s vybraným problémem, aby šla chyba rychleji dohledat.', reportHelpButton:'Nahlásit tento problém ↗'
  ...
  de: {
    ...
    sideInstallerTitle:'SideStore ohne PC auf iOS 27 installieren', sideInstallerDesc:'SideInstaller installiert SideStore oder SideStore + LiveContainer unter iOS/iPadOS 27 vollständig auf dem Gerät. Die aktuellen offiziellen Anforderungen nennen iOS 27, Wi-Fi für die Ersteinrichtung und keinen PC.', sideInstallerSafety:'Bei der Eingabe deines Apple-Accounts nur sideinstaller.net oder das offizielle GitHub-Repository FrizzleM/SideInstaller verwenden.', sideInstallerOpen:'Offiziellen SideInstaller öffnen ↗',
    sideInstallerBest:'SideStore / LiveContainer direkt auf dem Gerät einrichten', sideInstallerDevice:'iOS/iPadOS 27', sideInstallerComputer:'Unter iOS/iPadOS 27 kein PC; Wi-Fi ist für die Ersteinrichtung erforderlich', sideInstallerRefresh:'Nur Installer; die Aktualisierung übernimmt danach SideStore',
...
reportHelpTitle:'Funktioniert es immer noch nicht?', reportHelpDesc:'Öffne einen vorausgefüllten GitHub-Bericht mit dem ausgewählten Problem.', reportHelpButton:'Problem melden ↗'
  ...
  es: {
    ...
    sideInstallerTitle:'Instalar SideStore sin PC en iOS 27', sideInstallerDesc:'SideInstaller instala SideStore o SideStore + LiveContainer completamente desde el dispositivo en iOS/iPadOS 27. Los requisitos oficiales actuales indican iOS 27, Wi-Fi para la configuración inicial y ningún PC.', sideInstallerSafety:'Al introducir tu cuenta Apple usa solo sideinstaller.net o el repositorio oficial FrizzleM/SideInstaller en GitHub.', sideInstallerOpen:'Abrir SideInstaller oficial ↗',
    sideInstallerBest:'Configurar SideStore / LiveContainer desde el dispositivo', sideInstallerDevice:'iOS/iPadOS 27', sideInstallerComputer:'Sin PC en iOS/iPadOS 27; se necesita Wi-Fi para la primera configuración', sideInstallerRefresh:'Solo instala; después la renovación la gestiona SideStore',
...
reportHelpTitle:'¿Sigue sin funcionar?', reportHelpDesc:'Abre un informe de GitHub pre-rellenado con el problema seleccionado.', reportHelpButton:'Reportar este problema ↗'
  ...
  fr: {
    ...
    sideInstallerTitle:'Installer SideStore sans PC sur iOS 27', sideInstallerDesc:'SideInstaller installe SideStore ou SideStore + LiveContainer entièrement sur l’appareil sous iOS/iPadOS 27. Les exigences officielles actuelles indiquent iOS 27, le Wi-Fi pour la première configuration et aucun PC.', sideInstallerSafety:'Pour saisir votre compte Apple, utilisez uniquement sideinstaller.net ou le dépôt GitHub officiel FrizzleM/SideInstaller.', sideInstallerOpen:'Ouvrir SideInstaller officiel ↗',
    sideInstallerBest:'Configurer SideStore / LiveContainer sur l’appareil', sideInstallerDevice:'iOS/iPadOS 27', sideInstallerComputer:'Pas de PC sous iOS/iPadOS 27 ; le Wi-Fi est requis pour la première configuration', sideInstallerRefresh:'Installateur uniquement ; l’actualisation est ensuite gérée par SideStore',
...
const SETUP_RESULTS = {
  en: {
    ...
    pairingNeeded:{title:'iOS 17–26 without a PC: one-time computer setup is required',text:'The current official SideInstaller requirements list iOS 27 only. On iOS 17–26, use the standard SideStore path: you need pairing data created with a computer before on-device refresh can work.',steps:['Use a computer once to create the pairing data.','Complete the standard SideStore setup.','After setup, use SideStore + LocalDevVPN on-device.'],primary:'SideStore pairing guide ↗',url:'https://docs.sidestore.io/docs/advanced/pairing-file',secondary:'SideInstaller requirements ↗',secondaryUrl:'https://github.com/FrizzleM/SideInstaller#requirements'},
    checkVersionNoPc:{title:'Check your iOS version first',text:'The official no-PC SideInstaller route currently requires iOS 27. If you are on an older version, the standard SideStore setup may require a computer for pairing.',steps:['Open Settings → General → About.','Check the exact iOS/iPadOS version.','Choose the matching version in this wizard.'],primary:'SideInstaller requirements ↗',url:'https://github.com/FrizzleM/SideInstaller#requirements'},
  ...
  cs: {
    ...
    pairingNeeded:{title:'iOS 17–26 bez PC: pro první nastavení je potřeba počítač',text:'Aktuální oficiální požadavky SideInstalleru uvádějí pouze iOS 27. Na iOS 17–26 použij standardní cestu SideStore: pairing data je potřeba nejdřív vytvořit pomocí počítače.',steps:['Jednou použij počítač k vytvoření pairing dat.','Dokonči standardní nastavení SideStore.','Potom používej SideStore + LocalDevVPN přímo v zařízení.'],primary:'Návod k pairingu SideStore ↗',url:'https://docs.sidestore.io/docs/advanced/pairing-file',secondary:'Požadavky SideInstaller ↗',secondaryUrl:'https://github.com/FrizzleM/SideInstaller#requirements'},
    checkVersionNoPc:{title:'Nejdřív zjisti přesnou verzi iOS',text:'Oficiální cesta SideInstaller bez PC aktuálně vyžaduje iOS 27. Na starší verzi může standardní nastavení SideStore vyžadovat počítač kvůli pairingu.',steps:['Otevři Nastavení → Obecné → Informace.','Zjisti přesnou verzi iOS/iPadOS.','V průvodci vyber odpovídající verzi.'],primary:'Požadavky SideInstaller ↗',url:'https://github.com/FrizzleM/SideInstaller#requirements'},
  ...
  de: {
    ...
    pairingNeeded:{title:'iOS 17–26 ohne PC: einmalige Computer-Einrichtung erforderlich',text:'Die aktuellen offiziellen SideInstaller-Anforderungen nennen nur iOS 27. Unter iOS 17–26 nutze den normalen SideStore-Weg: Pairing-Daten müssen zuerst mit einem Computer erstellt werden.',steps:['Einmal einen Computer für die Pairing-Daten verwenden.','Das normale SideStore-Setup abschließen.','Danach SideStore + LocalDevVPN direkt auf dem Gerät nutzen.'],primary:'SideStore Pairing-Anleitung ↗',url:'https://docs.sidestore.io/docs/advanced/pairing-file',secondary:'SideInstaller Anforderungen ↗',secondaryUrl:'https://github.com/FrizzleM/SideInstaller#requirements'},
    checkVersionNoPc:{title:'Zuerst die genaue iOS-Version prüfen',text:'Der offizielle SideInstaller-Weg ohne PC erfordert derzeit iOS 27. Auf älteren Versionen kann das normale SideStore-Setup für das Pairing einen Computer benötigen.',steps:['Einstellungen → Allgemein → Info öffnen.','Genaue iOS/iPadOS-Version prüfen.','Danach die passende Version im Assistenten auswählen.'],primary:'SideInstaller Anforderungen ↗',url:'https://github.com/FrizzleM/SideInstaller#requirements'},
  ...
  es: {
    ...
    pairingNeeded:{title:'iOS 17–26 sin PC: se necesita un ordenador para la configuración inicial',text:'Los requisitos oficiales actuales de SideInstaller solo indican iOS 27. En iOS 17–26 usa la ruta estándar de SideStore: los datos de pairing deben crearse primero con un ordenador.',steps:['Usa un ordenador una vez para crear los datos de pairing.','Completa la configuración estándar de SideStore.','Después usa SideStore + LocalDevVPN desde el dispositivo.'],primary:'Guía de pairing SideStore ↗',url:'https://docs.sidestore.io/docs/advanced/pairing-file',secondary:'Requisitos de SideInstaller ↗',secondaryUrl:'https://github.com/FrizzleM/SideInstaller#requirements'},
    checkVersionNoPc:{title:'Comprueba primero tu versión exacta de iOS',text:'La ruta oficial de SideInstaller sin PC requiere actualmente iOS 27. En versiones anteriores, la configuración estándar de SideStore puede necesitar un ordenador para el pairing.',steps:['Abre Ajustes → General → Información.','Comprueba la versión exacta de iOS/iPadOS.','Después elige la versión correspondiente en este asistente.'],primary:'Requisitos de SideInstaller ↗',url:'https://github.com/FrizzleM/SideInstaller#requirements'},
  ...
  fr: {
    ...
    pairingNeeded:{title:'iOS 17–26 sans PC : un ordinateur est requis pour la configuration initiale',text:'Les exigences officielles actuelles de SideInstaller indiquent uniquement iOS 27. Sous iOS 17–26, utilisez la voie SideStore standard : les données d’appairage doivent d’abord être créées avec un ordinateur.',steps:['Utiliser un ordinateur une fois pour créer les données d’appairage.','Terminer la configuration SideStore standard.','Ensuite utiliser SideStore + LocalDevVPN directement sur l’appareil.'],primary:'Guide d’appairage SideStore ↗',url:'https://docs.sidestore.io/docs/advanced/pairing-file',secondary:'Prérequis SideInstaller ↗',secondaryUrl:'https://github.com/FrizzleM/SideInstaller#requirements'},
    checkVersionNoPc:{title:'Vérifiez d’abord la version exacte d’iOS',text:'La voie officielle SideInstaller sans PC nécessite actuellement iOS 27. Sur une version antérieure, la configuration SideStore standard peut nécessiter un ordinateur pour l’appairage.',steps:['Ouvrir Réglages → Général → Informations.','Vérifier la version exacte d’iOS/iPadOS.','Choisir ensuite la version correspondante dans cet assistant.'],primary:'Prérequis SideInstaller ↗',url:'https://github.com/FrizzleM/SideInstaller#requirements'},
