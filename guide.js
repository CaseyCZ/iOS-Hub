...
const GUIDE_COPY = {
  en: {
    ...
    sideInstallerTitle:'Install SideStore without a PC on iOS 27', sideInstallerDesc:'SideInstaller installs SideStore or SideStore + LiveContainer fully on-device on iOS/iPadOS 27. The current official SideInstaller requirements list iOS/iPadOS 27; for iOS 17–26 use the regular SideStore setup with a computer for initial pairing.', sideInstallerSafety:'Use only sideinstaller.net or the official FrizzleM/SideInstaller GitHub repository when entering Apple account credentials.', sideInstallerOpen:'Open official SideInstaller ↗',
    sideInstallerBest:'On-device SideStore / LiveContainer setup', sideInstallerDevice:'iOS/iPadOS 27', sideInstallerComputer:'No PC on iOS/iPadOS 27; use the regular SideStore setup on older versions', sideInstallerRefresh:'Installer only; refreshing is then handled by SideStore',
...
reportHelpTitle:'Still not working?', reportHelpDesc:'Open a pre-filled GitHub report with the selected problem so it is easier to diagnose.', reportHelpButton:'Report this problem ↗'
  ...
  cs: {
    ...
    sideInstallerTitle:'SideStore bez PC na iOS 27', sideInstallerDesc:'SideInstaller umí na iOS/iPadOS 27 nainstalovat SideStore nebo SideStore + LiveContainer přímo v zařízení. Aktuální oficiální požadavky SideInstalleru uvádějí iOS/iPadOS 27; na iOS 17–26 použij běžné první nastavení SideStore přes počítač.', sideInstallerSafety:'Při zadávání Apple účtu používej pouze sideinstaller.net nebo oficiální GitHub FrizzleM/SideInstaller.', sideInstallerOpen:'Otevřít oficiální SideInstaller ↗',
    sideInstallerBest:'Instalace SideStore / LiveContainer přímo v zařízení', sideInstallerDevice:'iOS/iPadOS 27', sideInstallerComputer:'Na iOS/iPadOS 27 bez PC; na starších verzích použij běžné nastavení SideStore', sideInstallerRefresh:'Je to instalátor; další obnovování řeší SideStore',
...
reportHelpTitle:'Pořád to nejde?', reportHelpDesc:'Otevři předvyplněné hlášení na GitHubu s vybraným problémem, aby šla chyba rychleji dohledat.', reportHelpButton:'Nahlásit tento problém ↗'
  ...
  de: {
    ...
    sideInstallerTitle:'SideStore ohne PC auf iOS 27 installieren', sideInstallerDesc:'SideInstaller installiert SideStore oder SideStore + LiveContainer unter iOS/iPadOS 27 vollständig auf dem Gerät. Die aktuellen offiziellen SideInstaller-Anforderungen nennen iOS/iPadOS 27; unter iOS 17–26 nutze das normale SideStore-Erstsetup mit einem Computer.', sideInstallerSafety:'Bei der Eingabe deines Apple-Accounts nur sideinstaller.net oder das offizielle GitHub-Repository FrizzleM/SideInstaller verwenden.', sideInstallerOpen:'Offiziellen SideInstaller öffnen ↗',
    sideInstallerBest:'SideStore / LiveContainer direkt auf dem Gerät einrichten', sideInstallerDevice:'iOS/iPadOS 27', sideInstallerComputer:'Unter iOS/iPadOS 27 kein PC; auf älteren Versionen das normale SideStore-Setup verwenden', sideInstallerRefresh:'Nur Installer; die Aktualisierung übernimmt danach SideStore',
...
reportHelpTitle:'Funktioniert es immer noch nicht?', reportHelpDesc:'Öffne einen vorausgefüllten GitHub-Bericht mit dem ausgewählten Problem.', reportHelpButton:'Problem melden ↗'
  ...
  es: {
    ...
    sideInstallerTitle:'Instalar SideStore sin PC en iOS 27', sideInstallerDesc:'SideInstaller instala SideStore o SideStore + LiveContainer completamente desde el dispositivo en iOS/iPadOS 27. Los requisitos oficiales actuales de SideInstaller indican iOS/iPadOS 27; en iOS 17–26 usa la configuración inicial normal de SideStore con un ordenador.', sideInstallerSafety:'Al introducir tu cuenta Apple usa solo sideinstaller.net o el repositorio oficial FrizzleM/SideInstaller en GitHub.', sideInstallerOpen:'Abrir SideInstaller oficial ↗',
    sideInstallerBest:'Configurar SideStore / LiveContainer desde el dispositivo', sideInstallerDevice:'iOS/iPadOS 27', sideInstallerComputer:'Sin PC en iOS/iPadOS 27; en versiones anteriores usa la configuración normal de SideStore', sideInstallerRefresh:'Solo instala; después la renovación la gestiona SideStore',
...
reportHelpTitle:'¿Sigue sin funcionar?', reportHelpDesc:'Abre un informe de GitHub pre-rellenado con el problema seleccionado.', reportHelpButton:'Reportar este problema ↗'
  ...
  fr: {
    ...
    sideInstallerTitle:'Installer SideStore sans PC sur iOS 27', sideInstallerDesc:'SideInstaller installe SideStore ou SideStore + LiveContainer entièrement sur l’appareil sous iOS/iPadOS 27. Les exigences officielles actuelles de SideInstaller indiquent iOS/iPadOS 27 ; sous iOS 17–26, utilisez la configuration initiale normale de SideStore avec un ordinateur.', sideInstallerSafety:'Pour saisir votre compte Apple, utilisez uniquement sideinstaller.net ou le dépôt GitHub officiel FrizzleM/SideInstaller.', sideInstallerOpen:'Ouvrir SideInstaller officiel ↗',
    sideInstallerBest:'Configurer SideStore / LiveContainer sur l’appareil', sideInstallerDevice:'iOS/iPadOS 27', sideInstallerComputer:'Pas de PC sous iOS/iPadOS 27 ; sur les anciennes versions, utilisez la configuration normale de SideStore', sideInstallerRefresh:'Installateur uniquement ; l’actualisation est ensuite gérée par SideStore',
...
const SETUP_RESULTS = {
  en: {
    ...
    pairingNeeded:{title:'iOS 17–26: one-time computer setup required',text:'The current official SideInstaller requirements list iOS/iPadOS 27. On iOS 17–26, use the regular SideStore setup and create the required pairing data with a computer once.',steps:['Use iloader or the current SideStore setup on a computer.','Create and place the pairing file for SideStore.','After setup, use SideStore + LocalDevVPN on-device.'],primary:'SideStore prerequisites ↗',url:'https://docs.sidestore.io/docs/installation/prerequisites',secondary:'Pairing guide ↗',secondaryUrl:'https://docs.sidestore.io/docs/advanced/pairing-file'},
    checkVersionNoPc:{title:'Check your iOS version first',text:'The no-PC path depends on the exact system version. The current official SideInstaller requirements list iOS/iPadOS 27. On iOS 17–26, plan a one-time computer setup for SideStore.',steps:['Open Settings → General → About.','Check the exact iOS/iPadOS version.','Choose iOS 27 or iOS 17–26 in this wizard.'],primary:'SideInstaller requirements ↗',url:'https://github.com/FrizzleM/SideInstaller',secondary:'SideStore prerequisites ↗',secondaryUrl:'https://docs.sidestore.io/docs/installation/prerequisites'},
  ...
  cs: {
    ...
    pairingNeeded:{title:'iOS 17–26: první nastavení vyžaduje počítač',text:'Aktuální oficiální požadavky SideInstalleru uvádějí iOS/iPadOS 27. Na iOS 17–26 použij běžné nastavení SideStore a jednorázově vytvoř potřebná pairing data na počítači.',steps:['Použij iloader nebo aktuální postup SideStore na počítači.','Vytvoř a vlož pairing soubor pro SideStore.','Po nastavení používej SideStore + LocalDevVPN přímo v zařízení.'],primary:'Požadavky SideStore ↗',url:'https://docs.sidestore.io/docs/installation/prerequisites',secondary:'Návod k pairingu ↗',secondaryUrl:'https://docs.sidestore.io/docs/advanced/pairing-file'},
    checkVersionNoPc:{title:'Nejdřív zjisti přesnou verzi iOS',text:'Cesta bez PC závisí na přesné verzi systému. Aktuální oficiální požadavky SideInstalleru uvádějí iOS/iPadOS 27. Na iOS 17–26 počítej s jednorázovým nastavením SideStore přes počítač.',steps:['Otevři Nastavení → Obecné → Informace.','Zjisti přesnou verzi iOS/iPadOS.','V průvodci pak vyber iOS 27 nebo iOS 17–26.'],primary:'Požadavky SideInstaller ↗',url:'https://github.com/FrizzleM/SideInstaller',secondary:'Požadavky SideStore ↗',secondaryUrl:'https://docs.sidestore.io/docs/installation/prerequisites'},
  ...
  de: {
    ...
    pairingNeeded:{title:'iOS 17–26: einmaliges Computer-Setup erforderlich',text:'Die aktuellen offiziellen SideInstaller-Anforderungen nennen iOS/iPadOS 27. Unter iOS 17–26 nutze das normale SideStore-Setup und erstelle die benötigten Pairing-Daten einmal mit einem Computer.',steps:['iloader oder das aktuelle SideStore-Setup auf einem Computer verwenden.','Pairing-Datei für SideStore erstellen und ablegen.','Danach SideStore + LocalDevVPN direkt auf dem Gerät verwenden.'],primary:'SideStore Voraussetzungen ↗',url:'https://docs.sidestore.io/docs/installation/prerequisites',secondary:'Pairing-Anleitung ↗',secondaryUrl:'https://docs.sidestore.io/docs/advanced/pairing-file'},
    checkVersionNoPc:{title:'Zuerst die genaue iOS-Version prüfen',text:'Der Weg ohne PC hängt von der exakten Systemversion ab. Die aktuellen offiziellen SideInstaller-Anforderungen nennen iOS/iPadOS 27. Unter iOS 17–26 ist ein einmaliges Computer-Setup für SideStore einzuplanen.',steps:['Einstellungen → Allgemein → Info öffnen.','Genaue iOS/iPadOS-Version prüfen.','Danach im Assistenten iOS 27 oder iOS 17–26 auswählen.'],primary:'SideInstaller Anforderungen ↗',url:'https://github.com/FrizzleM/SideInstaller',secondary:'SideStore Voraussetzungen ↗',secondaryUrl:'https://docs.sidestore.io/docs/installation/prerequisites'},
  ...
  es: {
    ...
    pairingNeeded:{title:'iOS 17–26: se necesita un ordenador una vez',text:'Los requisitos oficiales actuales de SideInstaller indican iOS/iPadOS 27. En iOS 17–26 usa la configuración normal de SideStore y crea una vez los datos de pairing necesarios con un ordenador.',steps:['Usa iloader o la configuración actual de SideStore en un ordenador.','Crea y coloca el archivo de pairing para SideStore.','Después usa SideStore + LocalDevVPN desde el dispositivo.'],primary:'Requisitos de SideStore ↗',url:'https://docs.sidestore.io/docs/installation/prerequisites',secondary:'Guía de pairing ↗',secondaryUrl:'https://docs.sidestore.io/docs/advanced/pairing-file'},
    checkVersionNoPc:{title:'Comprueba primero tu versión exacta de iOS',text:'La ruta sin PC depende de la versión exacta. Los requisitos oficiales actuales de SideInstaller indican iOS/iPadOS 27. En iOS 17–26, prevé una configuración inicial de SideStore con un ordenador.',steps:['Abre Ajustes → General → Información.','Comprueba la versión exacta de iOS/iPadOS.','Después elige iOS 27 o iOS 17–26 en este asistente.'],primary:'Requisitos de SideInstaller ↗',url:'https://github.com/FrizzleM/SideInstaller',secondary:'Requisitos de SideStore ↗',secondaryUrl:'https://docs.sidestore.io/docs/installation/prerequisites'},
  ...
  fr: {
    ...
    pairingNeeded:{title:'iOS 17–26 : configuration unique sur ordinateur requise',text:'Les exigences officielles actuelles de SideInstaller indiquent iOS/iPadOS 27. Sous iOS 17–26, utilisez la configuration normale de SideStore et créez une fois les données d’appairage nécessaires avec un ordinateur.',steps:['Utiliser iloader ou la configuration SideStore actuelle sur un ordinateur.','Créer et placer le fichier d’appairage pour SideStore.','Après la configuration, utiliser SideStore + LocalDevVPN sur l’appareil.'],primary:'Prérequis SideStore ↗',url:'https://docs.sidestore.io/docs/installation/prerequisites',secondary:'Guide d’appairage ↗',secondaryUrl:'https://docs.sidestore.io/docs/advanced/pairing-file'},
    checkVersionNoPc:{title:'Vérifiez d’abord la version exacte d’iOS',text:'Le parcours sans PC dépend de la version exacte du système. Les exigences officielles actuelles de SideInstaller indiquent iOS/iPadOS 27. Sous iOS 17–26, prévoyez une configuration initiale de SideStore avec un ordinateur.',steps:['Ouvrir Réglages → Général → Informations.','Vérifier la version exacte d’iOS/iPadOS.','Choisir ensuite iOS 27 ou iOS 17–26 dans cet assistant.'],primary:'Prérequis SideInstaller ↗',url:'https://github.com/FrizzleM/SideInstaller',secondary:'Prérequis SideStore ↗',secondaryUrl:'https://docs.sidestore.io/docs/installation/prerequisites'},
