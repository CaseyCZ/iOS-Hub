import { SUPPORTED_LANGUAGES, applyTranslations, normalizeLanguage, t } from './i18n.js?v=1.1.5-20260918-audit10';
...
const GUIDE_COPY = {
  en: {
    ...
    sideInstallerTitle:'Install SideStore without a PC on iOS 27', sideInstallerDesc:'SideInstaller installs SideStore or SideStore + LiveContainer fully on-device on iOS/iPadOS 27. SideInstaller v1.0.0 also adds partial iOS/iPadOS 17–26 support when you already have a pairing file created with a computer.', sideInstallerSafety:'Use only sideinstaller.net or the official FrizzleM/SideInstaller GitHub repository when entering Apple account credentials.', sideInstallerOpen:'Open official SideInstaller ↗',
    sideInstallerBest:'On-device SideStore / LiveContainer setup', sideInstallerDevice:'iOS/iPadOS 27; partial 17–26 support with an existing pairing file', sideInstallerComputer:'No PC on iOS/iPadOS 27; iOS/iPadOS 17–26 requires an existing pairing file created with a computer', sideInstallerRefresh:'Installer only; refreshing is then handled by SideStore',
...
reportHelpTitle:'Still not working?', reportHelpDesc:'Open a pre-filled GitHub report with the selected problem so it is easier to diagnose.', reportHelpButton:'Report this problem ↗'
  ...
  cs: {
    ...
    sideInstallerTitle:'SideStore bez PC na iOS 27', sideInstallerDesc:'SideInstaller umí na iOS/iPadOS 27 nainstalovat SideStore nebo SideStore + LiveContainer přímo v zařízení. SideInstaller v1.0.0 navíc přidává částečnou podporu iOS/iPadOS 17–26, pokud už máš pairing soubor vytvořený pomocí počítače.', sideInstallerSafety:'Při zadávání Apple účtu používej pouze sideinstaller.net nebo oficiální GitHub FrizzleM/SideInstaller.', sideInstallerOpen:'Otevřít oficiální SideInstaller ↗',
    sideInstallerBest:'Instalace SideStore / LiveContainer přímo v zařízení', sideInstallerDevice:'iOS/iPadOS 27; částečná podpora 17–26 s existujícím pairing souborem', sideInstallerComputer:'Na iOS/iPadOS 27 bez PC; iOS/iPadOS 17–26 vyžaduje existující pairing soubor vytvořený pomocí počítače', sideInstallerRefresh:'Je to instalátor; další obnovování řeší SideStore',
    ...
    troubleLoginQ:'Selže přihlášení Apple ID, HTTP 503 nebo 2FA', troubleLoginA:'U problémů s přihlášením v SideStore nejdřív změň Anisette Server. Pokud nepřijde ověřovací kód Apple, vyžádej si ho ručně v nastavení Apple účtu. Neopakuj přihlášení rychle za sebou a používej aktuální oficiální build SideStore.',
...
reportHelpTitle:'Funktioniert es immer noch nicht?', reportHelpDesc:'Öffne einen vorausgefüllten GitHub-Bericht mit dem ausgewählten Problem.', reportHelpButton:'Problem melden ↗'
  ...
  es: {
    ...
    sideInstallerTitle:'Instalar SideStore sin PC en iOS 27', sideInstallerDesc:'SideInstaller instala SideStore o SideStore + LiveContainer completamente desde el dispositivo en iOS/iPadOS 27. SideInstaller v1.0.0 también añade compatibilidad parcial con iOS/iPadOS 17–26 si ya tienes un archivo de pairing creado con un ordenador.', sideInstallerSafety:'Al introducir tu cuenta Apple usa solo sideinstaller.net o el repositorio oficial FrizzleM/SideInstaller en GitHub.', sideInstallerOpen:'Abrir SideInstaller oficial ↗',
    sideInstallerBest:'Configurar SideStore / LiveContainer desde el dispositivo', sideInstallerDevice:'iOS/iPadOS 27; compatibilidad parcial 17–26 con archivo de pairing existente', sideInstallerComputer:'Sin PC en iOS/iPadOS 27; iOS/iPadOS 17–26 necesita un archivo de pairing existente creado con un ordenador', sideInstallerRefresh:'Solo instala; después la renovación la gestiona SideStore',
    ...
    troubleLoginQ:'Falla el acceso con Apple ID, HTTP 503 o 2FA', troubleLoginA:'Para problemas de acceso en SideStore, prueba primero otro servidor Anisette. Si no llega el código de verificación de Apple, solicítalo manualmente en los ajustes de la cuenta Apple. Evita intentos rápidos y repetidos de inicio de sesión y usa la versión oficial más reciente de SideStore.',
...
reportHelpTitle:'¿Sigue sin funcionar?', reportHelpDesc:'Abre un informe de GitHub pre-rellenado con el problema seleccionado.', reportHelpButton:'Reportar este problema ↗'
  ...
  fr: {
    ...
    sideInstallerTitle:'Installer SideStore sans PC sur iOS 27', sideInstallerDesc:'SideInstaller installe SideStore ou SideStore + LiveContainer entièrement sur l’appareil sous iOS/iPadOS 27. SideInstaller v1.0.0 ajoute aussi une prise en charge partielle d’iOS/iPadOS 17–26 si vous disposez déjà d’un fichier d’appairage créé avec un ordinateur.', sideInstallerSafety:'Pour saisir votre compte Apple, utilisez uniquement sideinstaller.net ou le dépôt GitHub officiel FrizzleM/SideInstaller.', sideInstallerOpen:'Ouvrir SideInstaller officiel ↗',
    sideInstallerBest:'Configurer SideStore / LiveContainer sur l’appareil', sideInstallerDevice:'iOS/iPadOS 27 ; prise en charge partielle 17–26 avec fichier d’appairage existant', sideInstallerComputer:'Pas de PC sous iOS/iPadOS 27 ; iOS/iPadOS 17–26 nécessite un fichier d’appairage existant créé avec un ordinateur', sideInstallerRefresh:'Installateur uniquement ; l’actualisation est ensuite gérée par SideStore',
...
const SETUP_RESULTS = {
  en: {
    ...
    pairingNeeded:{title:'iOS 17–26: SideInstaller v1 needs an existing pairing file',text:'SideInstaller v1.0.0 officially adds partial iOS/iPadOS 17–26 support, but you must already have your pairing file. If you do not have one, a computer is needed once to create it.',steps:['Check whether you already have a valid pairing file.','If not, create one with a computer, then keep that file available.','Use SideInstaller v1 or the standard SideStore path with the pairing data.'],primary:'SideInstaller v1.0.0 release ↗',url:'https://github.com/FrizzleM/SideInstaller/releases/tag/v1.0.0',secondary:'SideStore pairing guide ↗',secondaryUrl:'https://docs.sidestore.io/docs/advanced/pairing-file'},
    checkVersionNoPc:{title:'Check your iOS version first',text:'On iOS/iPadOS 27, SideInstaller works fully on-device. SideInstaller v1.0.0 also supports iOS/iPadOS 17–26 partially when an existing pairing file is available.',steps:['Open Settings → General → About.','Check the exact iOS/iPadOS version.','Choose iOS 27 or iOS 17–26 in this wizard; on 17–26 you need an existing pairing file.'],primary:'SideInstaller v1.0.0 release ↗',url:'https://github.com/FrizzleM/SideInstaller/releases/tag/v1.0.0'},
  ...
  cs: {
    ...
    pairingNeeded:{title:'iOS 17–26: SideInstaller v1 potřebuje existující pairing soubor',text:'SideInstaller v1.0.0 oficiálně přidává částečnou podporu iOS/iPadOS 17–26, ale pairing soubor už musíš mít. Pokud ho nemáš, je jednorázově potřeba počítač k jeho vytvoření.',steps:['Ověř, jestli už máš platný pairing soubor.','Pokud ne, vytvoř ho pomocí počítače a soubor si ponech.','Potom použij SideInstaller v1 nebo standardní SideStore s těmito pairing daty.'],primary:'SideInstaller v1.0.0 release ↗',url:'https://github.com/FrizzleM/SideInstaller/releases/tag/v1.0.0',secondary:'Návod k pairingu SideStore ↗',secondaryUrl:'https://docs.sidestore.io/docs/advanced/pairing-file'},
    checkVersionNoPc:{title:'Nejdřív zjisti přesnou verzi iOS',text:'Na iOS/iPadOS 27 funguje SideInstaller kompletně v zařízení. SideInstaller v1.0.0 navíc částečně podporuje iOS/iPadOS 17–26, pokud už máš existující pairing soubor.',steps:['Otevři Nastavení → Obecné → Informace.','Zjisti přesnou verzi iOS/iPadOS.','Vyber iOS 27 nebo iOS 17–26; na 17–26 potřebuješ existující pairing soubor.'],primary:'SideInstaller v1.0.0 release ↗',url:'https://github.com/FrizzleM/SideInstaller/releases/tag/v1.0.0'},
  ...
  de: {
    ...
    pairingNeeded:{title:'iOS 17–26: SideInstaller v1 benötigt eine vorhandene Pairing-Datei',text:'SideInstaller v1.0.0 unterstützt iOS/iPadOS 17–26 offiziell teilweise, aber die Pairing-Datei muss bereits vorhanden sein. Wenn du noch keine hast, wird einmal ein Computer zum Erstellen benötigt.',steps:['Prüfen, ob bereits eine gültige Pairing-Datei vorhanden ist.','Falls nicht, sie einmal mit einem Computer erstellen und aufbewahren.','Danach SideInstaller v1 oder den normalen SideStore-Weg mit diesen Pairing-Daten verwenden.'],primary:'SideInstaller v1.0.0 Release ↗',url:'https://github.com/FrizzleM/SideInstaller/releases/tag/v1.0.0',secondary:'SideStore Pairing-Anleitung ↗',secondaryUrl:'https://docs.sidestore.io/docs/advanced/pairing-file'},
    checkVersionNoPc:{title:'Zuerst die genaue iOS-Version prüfen',text:'Unter iOS/iPadOS 27 funktioniert SideInstaller vollständig auf dem Gerät. SideInstaller v1.0.0 unterstützt iOS/iPadOS 17–26 außerdem teilweise, wenn bereits eine Pairing-Datei vorhanden ist.',steps:['Einstellungen → Allgemein → Info öffnen.','Genaue iOS/iPadOS-Version prüfen.','iOS 27 oder iOS 17–26 auswählen; unter 17–26 wird eine vorhandene Pairing-Datei benötigt.'],primary:'SideInstaller v1.0.0 Release ↗',url:'https://github.com/FrizzleM/SideInstaller/releases/tag/v1.0.0'},
  ...
  es: {
    ...
    pairingNeeded:{title:'iOS 17–26: SideInstaller v1 necesita un archivo de pairing existente',text:'SideInstaller v1.0.0 añade oficialmente compatibilidad parcial con iOS/iPadOS 17–26, pero el archivo de pairing debe existir previamente. Si no lo tienes, necesitarás un ordenador una vez para crearlo.',steps:['Comprueba si ya tienes un archivo de pairing válido.','Si no, créalo una vez con un ordenador y consérvalo.','Después usa SideInstaller v1 o SideStore estándar con esos datos de pairing.'],primary:'Release SideInstaller v1.0.0 ↗',url:'https://github.com/FrizzleM/SideInstaller/releases/tag/v1.0.0',secondary:'Guía de pairing SideStore ↗',secondaryUrl:'https://docs.sidestore.io/docs/advanced/pairing-file'},
    checkVersionNoPc:{title:'Comprueba primero tu versión exacta de iOS',text:'En iOS/iPadOS 27, SideInstaller funciona completamente en el dispositivo. SideInstaller v1.0.0 también admite parcialmente iOS/iPadOS 17–26 cuando ya existe un archivo de pairing.',steps:['Abre Ajustes → General → Información.','Comprueba la versión exacta de iOS/iPadOS.','Elige iOS 27 o iOS 17–26; en 17–26 necesitas un archivo de pairing existente.'],primary:'Release SideInstaller v1.0.0 ↗',url:'https://github.com/FrizzleM/SideInstaller/releases/tag/v1.0.0'},
  ...
  fr: {
    ...
    pairingNeeded:{title:'iOS 17–26 : SideInstaller v1 nécessite un fichier d’appairage existant',text:'SideInstaller v1.0.0 ajoute officiellement une prise en charge partielle d’iOS/iPadOS 17–26, mais le fichier d’appairage doit déjà exister. Si vous ne l’avez pas, un ordinateur est nécessaire une fois pour le créer.',steps:['Vérifier si vous avez déjà un fichier d’appairage valide.','Sinon, le créer une fois avec un ordinateur et le conserver.','Utiliser ensuite SideInstaller v1 ou SideStore standard avec ces données d’appairage.'],primary:'Release SideInstaller v1.0.0 ↗',url:'https://github.com/FrizzleM/SideInstaller/releases/tag/v1.0.0',secondary:'Guide d’appairage SideStore ↗',secondaryUrl:'https://docs.sidestore.io/docs/advanced/pairing-file'},
    checkVersionNoPc:{title:'Vérifiez d’abord la version exacte d’iOS',text:'Sous iOS/iPadOS 27, SideInstaller fonctionne entièrement sur l’appareil. SideInstaller v1.0.0 prend aussi partiellement en charge iOS/iPadOS 17–26 lorsqu’un fichier d’appairage existe déjà.',steps:['Ouvrir Réglages → Général → Informations.','Vérifier la version exacte d’iOS/iPadOS.','Choisir iOS 27 ou iOS 17–26 ; sous 17–26 un fichier d’appairage existant est nécessaire.'],primary:'Release SideInstaller v1.0.0 ↗',url:'https://github.com/FrizzleM/SideInstaller/releases/tag/v1.0.0'},
