import { SUPPORTED_LANGUAGES, applyTranslations, normalizeLanguage, t } from './i18n.js?v=1.1.5-20260918-audit3';
...
const GUIDE_COPY = {
  en: {
    ...
    sideInstallerTitle:'Install SideStore without a PC on iOS 27', sideInstallerDesc:'SideInstaller installs SideStore or SideStore + LiveContainer fully on-device. The current official requirements list iOS/iPadOS 27 only.', sideInstallerSafety:'Use only sideinstaller.net or the official FrizzleM/SideInstaller GitHub repository when entering Apple account credentials.', sideInstallerOpen:'Open official SideInstaller ↗',
    sideInstallerBest:'On-device SideStore / LiveContainer setup', sideInstallerDevice:'iOS/iPadOS 27', sideInstallerComputer:'No PC on iOS/iPadOS 27; earlier versions use the standard SideStore computer-based initial setup', sideInstallerRefresh:'Installer only; refreshing is then handled by SideStore',
...
reportHelpTitle:'Still not working?', reportHelpDesc:'Open a pre-filled GitHub report with the selected problem so it is easier to diagnose.', reportHelpButton:'Report this problem ↗'
  ...
  cs: {
    ...
    sideInstallerTitle:'SideStore bez PC na iOS 27', sideInstallerDesc:'SideInstaller umí nainstalovat SideStore nebo SideStore + LiveContainer přímo v zařízení. Aktuální oficiální požadavky uvádějí pouze iOS/iPadOS 27.', sideInstallerSafety:'Při zadávání Apple účtu používej pouze sideinstaller.net nebo oficiální GitHub FrizzleM/SideInstaller.', sideInstallerOpen:'Otevřít oficiální SideInstaller ↗',
    sideInstallerBest:'Instalace SideStore / LiveContainer přímo v zařízení', sideInstallerDevice:'iOS/iPadOS 27', sideInstallerComputer:'Na iOS/iPadOS 27 bez PC; starší verze používají standardní první nastavení SideStore přes počítač', sideInstallerRefresh:'Je to instalátor; další obnovování řeší SideStore',
    ...
    troubleLoginQ:'Selže přihlášení Apple ID, HTTP 503 nebo 2FA', troubleLoginA:'U problémů s přihlášením v SideStore nejdřív změň Anisette Server. Pokud nepřijde ověřovací kód Apple, vyžádej si ho ručně v nastavení Apple účtu. Pokud používáš SideInstaller, použij aktuální oficiální build a řiď se podrobnostmi chyby zobrazenými přímo v aplikaci.',
...
reportHelpTitle:'Pořád to nejde?', reportHelpDesc:'Otevři předvyplněné hlášení na GitHubu s vybraným problémem, aby šla chyba rychleji dohledat.', reportHelpButton:'Nahlásit tento problém ↗'
  ...
  de: {
    ...
    sideInstallerTitle:'SideStore ohne PC auf iOS 27 installieren', sideInstallerDesc:'SideInstaller installiert SideStore oder SideStore + LiveContainer vollständig auf dem Gerät. Die aktuellen offiziellen Anforderungen nennen nur iOS/iPadOS 27.', sideInstallerSafety:'Bei der Eingabe deines Apple-Accounts nur sideinstaller.net oder das offizielle GitHub-Repository FrizzleM/SideInstaller verwenden.', sideInstallerOpen:'Offiziellen SideInstaller öffnen ↗',
    sideInstallerBest:'SideStore / LiveContainer direkt auf dem Gerät einrichten', sideInstallerDevice:'iOS/iPadOS 27', sideInstallerComputer:'Unter iOS/iPadOS 27 kein PC; ältere Versionen verwenden das normale computerbasierte SideStore-Erstsetup', sideInstallerRefresh:'Nur Installer; die Aktualisierung übernimmt danach SideStore',
...
reportHelpTitle:'Funktioniert es immer noch nicht?', reportHelpDesc:'Öffne einen vorausgefüllten GitHub-Bericht mit dem ausgewählten Problem.', reportHelpButton:'Problem melden ↗'
  ...
  es: {
    ...
    sideInstallerTitle:'Instalar SideStore sin PC en iOS 27', sideInstallerDesc:'SideInstaller instala SideStore o SideStore + LiveContainer completamente desde el dispositivo. Los requisitos oficiales actuales solo indican iOS/iPadOS 27.', sideInstallerSafety:'Al introducir tu cuenta Apple usa solo sideinstaller.net o el repositorio oficial FrizzleM/SideInstaller en GitHub.', sideInstallerOpen:'Abrir SideInstaller oficial ↗',
    sideInstallerBest:'Configurar SideStore / LiveContainer desde el dispositivo', sideInstallerDevice:'iOS/iPadOS 27', sideInstallerComputer:'Sin PC en iOS/iPadOS 27; las versiones anteriores usan la configuración inicial estándar de SideStore con ordenador', sideInstallerRefresh:'Solo instala; después la renovación la gestiona SideStore',
    ...
    troubleLoginQ:'Falla el acceso con Apple ID, HTTP 503 o 2FA', troubleLoginA:'Para problemas de acceso en SideStore, prueba primero otro servidor Anisette. Si no llega el código de verificación de Apple, solicítalo manualmente en los ajustes de la cuenta Apple. Si usas SideInstaller, utiliza la versión oficial actual y sigue los detalles del error de acceso que muestra la app.',
...
reportHelpTitle:'¿Sigue sin funcionar?', reportHelpDesc:'Abre un informe de GitHub pre-rellenado con el problema seleccionado.', reportHelpButton:'Reportar este problema ↗'
  ...
  fr: {
    ...
    sideInstallerTitle:'Installer SideStore sans PC sur iOS 27', sideInstallerDesc:'SideInstaller installe SideStore ou SideStore + LiveContainer entièrement sur l’appareil. Les exigences officielles actuelles indiquent uniquement iOS/iPadOS 27.', sideInstallerSafety:'Pour saisir votre compte Apple, utilisez uniquement sideinstaller.net ou le dépôt GitHub officiel FrizzleM/SideInstaller.', sideInstallerOpen:'Ouvrir SideInstaller officiel ↗',
    sideInstallerBest:'Configurer SideStore / LiveContainer sur l’appareil', sideInstallerDevice:'iOS/iPadOS 27', sideInstallerComputer:'Pas de PC sous iOS/iPadOS 27 ; les versions antérieures utilisent la configuration initiale SideStore standard avec ordinateur', sideInstallerRefresh:'Installateur uniquement ; l’actualisation est ensuite gérée par SideStore',
...
const SETUP_RESULTS = {
  en: {
    ...
    pairingNeeded:{title:'iOS 17–26: standard SideStore setup needs a computer',text:'The current official SideInstaller requirements list iOS/iPadOS 27 only. On iOS 17–26, use the normal SideStore installation path and create the required pairing data with a computer.',steps:['Use the official SideStore installation guide.','Create the pairing data with a supported computer tool such as iloader.','After setup, use SideStore + LocalDevVPN on-device.'],primary:'Open SideStore installation ↗',url:'https://docs.sidestore.io/docs/installation/prerequisites',secondary:'Open iloader ↗',secondaryUrl:'https://iloader.app/'},
    checkVersionNoPc:{title:'Check your iOS version first',text:'The no-PC route depends on the exact system version. The current official SideInstaller requirements list iOS/iPadOS 27 only; earlier versions use the standard computer-based SideStore setup.',steps:['Open Settings → General → About.','Check the exact iOS/iPadOS version.','Choose the matching version in this wizard.'],primary:'SideInstaller requirements ↗',url:'https://github.com/FrizzleM/SideInstaller#requirements',secondary:'SideStore setup ↗',secondaryUrl:'https://docs.sidestore.io/docs/installation/prerequisites'},
  ...
  cs: {
    ...
    pairingNeeded:{title:'iOS 17–26: standardní SideStore potřebuje pro první nastavení počítač',text:'Aktuální oficiální požadavky SideInstalleru uvádějí pouze iOS/iPadOS 27. Na iOS 17–26 použij běžnou instalaci SideStore a vytvoř potřebná pairing data pomocí počítače.',steps:['Použij oficiální návod instalace SideStore.','Vytvoř pairing data podporovaným nástrojem na počítači, například iloaderem.','Po nastavení používej SideStore + LocalDevVPN přímo v zařízení.'],primary:'Otevřít instalaci SideStore ↗',url:'https://docs.sidestore.io/docs/installation/prerequisites',secondary:'Otevřít iloader ↗',secondaryUrl:'https://iloader.app/'},
    checkVersionNoPc:{title:'Nejdřív zjisti přesnou verzi iOS',text:'Cesta bez PC závisí na přesné verzi systému. Aktuální oficiální požadavky SideInstalleru uvádějí pouze iOS/iPadOS 27; starší verze používají standardní nastavení SideStore přes počítač.',steps:['Otevři Nastavení → Obecné → Informace.','Zjisti přesnou verzi iOS/iPadOS.','V průvodci vyber odpovídající verzi.'],primary:'Požadavky SideInstaller ↗',url:'https://github.com/FrizzleM/SideInstaller#requirements',secondary:'Nastavení SideStore ↗',secondaryUrl:'https://docs.sidestore.io/docs/installation/prerequisites'},
  ...
  de: {
    ...
    pairingNeeded:{title:'iOS 17–26: Für das normale SideStore-Erstsetup ist ein Computer nötig',text:'Die aktuellen offiziellen SideInstaller-Anforderungen nennen nur iOS/iPadOS 27. Unter iOS 17–26 verwende den normalen SideStore-Installationsweg und erstelle die erforderlichen Pairing-Daten mit einem Computer.',steps:['Die offizielle SideStore-Installationsanleitung verwenden.','Pairing-Daten mit einem unterstützten Computer-Tool wie iloader erstellen.','Danach SideStore + LocalDevVPN direkt auf dem Gerät verwenden.'],primary:'SideStore-Installation öffnen ↗',url:'https://docs.sidestore.io/docs/installation/prerequisites',secondary:'iloader öffnen ↗',secondaryUrl:'https://iloader.app/'},
    checkVersionNoPc:{title:'Zuerst die genaue iOS-Version prüfen',text:'Der Weg ohne PC hängt von der genauen Systemversion ab. Die aktuellen offiziellen SideInstaller-Anforderungen nennen nur iOS/iPadOS 27; ältere Versionen verwenden das normale computerbasierte SideStore-Setup.',steps:['Einstellungen → Allgemein → Info öffnen.','Genaue iOS/iPadOS-Version prüfen.','Danach im Assistenten die passende Version auswählen.'],primary:'SideInstaller-Anforderungen ↗',url:'https://github.com/FrizzleM/SideInstaller#requirements',secondary:'SideStore-Setup ↗',secondaryUrl:'https://docs.sidestore.io/docs/installation/prerequisites'},
  ...
  es: {
    ...
    pairingNeeded:{title:'iOS 17–26: la configuración estándar de SideStore necesita un ordenador',text:'Los requisitos oficiales actuales de SideInstaller solo indican iOS/iPadOS 27. En iOS 17–26 usa la instalación normal de SideStore y crea los datos de pairing necesarios con un ordenador.',steps:['Usa la guía oficial de instalación de SideStore.','Crea los datos de pairing con una herramienta compatible como iloader en un ordenador.','Después usa SideStore + LocalDevVPN desde el dispositivo.'],primary:'Abrir instalación de SideStore ↗',url:'https://docs.sidestore.io/docs/installation/prerequisites',secondary:'Abrir iloader ↗',secondaryUrl:'https://iloader.app/'},
    checkVersionNoPc:{title:'Comprueba primero tu versión exacta de iOS',text:'La ruta sin PC depende de la versión exacta. Los requisitos oficiales actuales de SideInstaller solo indican iOS/iPadOS 27; las versiones anteriores usan la configuración estándar de SideStore con ordenador.',steps:['Abre Ajustes → General → Información.','Comprueba la versión exacta de iOS/iPadOS.','Después elige la versión correspondiente en este asistente.'],primary:'Requisitos de SideInstaller ↗',url:'https://github.com/FrizzleM/SideInstaller#requirements',secondary:'Configurar SideStore ↗',secondaryUrl:'https://docs.sidestore.io/docs/installation/prerequisites'},
  ...
  fr: {
    ...
    pairingNeeded:{title:'iOS 17–26 : la configuration SideStore standard nécessite un ordinateur',text:'Les exigences officielles actuelles de SideInstaller indiquent uniquement iOS/iPadOS 27. Sous iOS 17–26, utilisez l’installation SideStore normale et créez les données de pairing nécessaires avec un ordinateur.',steps:['Suivre le guide officiel d’installation SideStore.','Créer les données de pairing avec un outil compatible comme iloader sur ordinateur.','Ensuite utiliser SideStore + LocalDevVPN directement sur l’appareil.'],primary:'Ouvrir l’installation SideStore ↗',url:'https://docs.sidestore.io/docs/installation/prerequisites',secondary:'Ouvrir iloader ↗',secondaryUrl:'https://iloader.app/'},
    checkVersionNoPc:{title:'Vérifiez d’abord la version exacte d’iOS',text:'Le parcours sans PC dépend de la version exacte du système. Les exigences officielles actuelles de SideInstaller indiquent uniquement iOS/iPadOS 27 ; les versions antérieures utilisent la configuration SideStore standard avec ordinateur.',steps:['Ouvrir Réglages → Général → Informations.','Vérifier la version exacte d’iOS/iPadOS.','Choisir ensuite la version correspondante dans cet assistant.'],primary:'Prérequis SideInstaller ↗',url:'https://github.com/FrizzleM/SideInstaller#requirements',secondary:'Configuration SideStore ↗',secondaryUrl:'https://docs.sidestore.io/docs/installation/prerequisites'},
