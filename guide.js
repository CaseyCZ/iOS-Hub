...
const GUIDE_COPY = {
  en: {
    ...
    sideInstallerTitle:'Install SideStore without a PC on iOS 27', sideInstallerDesc:'SideInstaller installs SideStore or SideStore + LiveContainer fully on-device on iOS/iPadOS 27. The current official requirements list iOS/iPadOS 27 only.', sideInstallerSafety:'Use only sideinstaller.net or the official FrizzleM/SideInstaller GitHub repository when entering Apple account credentials.', sideInstallerOpen:'Open official SideInstaller ↗',
    sideInstallerBest:'On-device SideStore / LiveContainer setup', sideInstallerDevice:'iOS/iPadOS 27', sideInstallerComputer:'No PC on iOS 27; other versions use the standard SideStore computer-based initial setup', sideInstallerRefresh:'Installer only; refreshing is then handled by SideStore',
...
reportHelpTitle:'Still not working?', reportHelpDesc:'Open a pre-filled GitHub report with the selected problem so it is easier to diagnose.', reportHelpButton:'Report this problem ↗'
  ...
  cs: {
    ...
    sideInstallerBest:'Instalace SideStore / LiveContainer přímo v zařízení', sideInstallerDevice:'iOS/iPadOS 27', sideInstallerComputer:'Na iOS 27 bez PC; u ostatních verzí použij standardní prvotní instalaci SideStore přes počítač', sideInstallerRefresh:'Je to instalátor; další obnovování řeší SideStore',
...
reportHelpTitle:'Pořád to nejde?', reportHelpDesc:'Otevři předvyplněné hlášení na GitHubu s vybraným problémem, aby šla chyba rychleji dohledat.', reportHelpButton:'Nahlásit tento problém ↗'
  ...
  de: {
    ...
    sideInstallerTitle:'SideStore ohne PC auf iOS 27 installieren', sideInstallerDesc:'SideInstaller installiert SideStore oder SideStore + LiveContainer unter iOS/iPadOS 27 vollständig auf dem Gerät. Die aktuellen offiziellen Anforderungen nennen nur iOS/iPadOS 27.', sideInstallerSafety:'Bei der Eingabe deines Apple-Accounts nur sideinstaller.net oder das offizielle GitHub-Repository FrizzleM/SideInstaller verwenden.', sideInstallerOpen:'Offiziellen SideInstaller öffnen ↗',
    sideInstallerBest:'SideStore / LiveContainer direkt auf dem Gerät einrichten', sideInstallerDevice:'iOS/iPadOS 27', sideInstallerComputer:'Unter iOS 27 kein PC; bei anderen Versionen die normale SideStore-Ersteinrichtung am Computer verwenden', sideInstallerRefresh:'Nur Installer; die Aktualisierung übernimmt danach SideStore',
...
reportHelpTitle:'Funktioniert es immer noch nicht?', reportHelpDesc:'Öffne einen vorausgefüllten GitHub-Bericht mit dem ausgewählten Problem.', reportHelpButton:'Problem melden ↗'
  ...
  es: {
    ...
    sideInstallerTitle:'Instalar SideStore sin PC en iOS 27', sideInstallerDesc:'SideInstaller instala SideStore o SideStore + LiveContainer completamente desde el dispositivo en iOS/iPadOS 27. Los requisitos oficiales actuales indican únicamente iOS/iPadOS 27.', sideInstallerSafety:'Al introducir tu cuenta Apple usa solo sideinstaller.net o el repositorio oficial FrizzleM/SideInstaller en GitHub.', sideInstallerOpen:'Abrir SideInstaller oficial ↗',
    sideInstallerBest:'Configurar SideStore / LiveContainer desde el dispositivo', sideInstallerDevice:'iOS/iPadOS 27', sideInstallerComputer:'Sin PC en iOS 27; para otras versiones usa la instalación inicial estándar de SideStore con ordenador', sideInstallerRefresh:'Solo instala; después la renovación la gestiona SideStore',
...
reportHelpTitle:'¿Sigue sin funcionar?', reportHelpDesc:'Abre un informe de GitHub pre-rellenado con el problema seleccionado.', reportHelpButton:'Reportar este problema ↗'
  ...
  fr: {
    ...
    sideInstallerTitle:'Installer SideStore sans PC sur iOS 27', sideInstallerDesc:'SideInstaller installe SideStore ou SideStore + LiveContainer entièrement sur l’appareil sous iOS/iPadOS 27. Les exigences officielles actuelles indiquent uniquement iOS/iPadOS 27.', sideInstallerSafety:'Pour saisir votre compte Apple, utilisez uniquement sideinstaller.net ou le dépôt GitHub officiel FrizzleM/SideInstaller.', sideInstallerOpen:'Ouvrir SideInstaller officiel ↗',
    sideInstallerBest:'Configurer SideStore / LiveContainer sur l’appareil', sideInstallerDevice:'iOS/iPadOS 27', sideInstallerComputer:'Pas de PC sous iOS 27 ; pour les autres versions, utiliser l’installation initiale SideStore standard avec un ordinateur', sideInstallerRefresh:'Installateur uniquement ; l’actualisation est ensuite gérée par SideStore',
...
const SETUP_RESULTS = {
  en: {
    ...
    pairingNeeded:{title:'iOS 17–26 without a PC: one computer setup is required',text:'The current official SideInstaller requirements are iOS/iPadOS 27 only. On iOS 17–26, use the standard SideStore initial installation with a computer; after setup, normal installs and refreshes can be handled on-device.',steps:['Get access to a computer for the initial setup.','Install SideStore with iloader using the official SideStore instructions.','Then use SideStore with LocalDevVPN on the device.'],primary:'Open SideStore install guide ↗',url:'https://docs.sidestore.io/docs/installation/install',secondary:'Open iloader ↗',secondaryUrl:'https://iloader.app/'},
    checkVersionNoPc:{title:'Check your iOS version first',text:'The no-PC SideInstaller route currently requires iOS/iPadOS 27. On other versions, plan on a one-time computer setup for SideStore.',steps:['Open Settings → General → About.','Check the exact iOS/iPadOS version.','Choose the matching version in this wizard.'],primary:'SideInstaller requirements ↗',url:'https://github.com/FrizzleM/SideInstaller',secondary:'SideStore prerequisites ↗',secondaryUrl:'https://docs.sidestore.io/docs/installation/prerequisites'},
  ...
  cs: {
    ...
    pairingNeeded:{title:'iOS 17–26 bez PC: pro první nastavení je potřeba počítač',text:'Aktuální oficiální požadavky SideInstalleru uvádějí pouze iOS/iPadOS 27. Na iOS 17–26 použij standardní prvotní instalaci SideStore přes počítač; potom už může instalace a refresh běžet přímo v zařízení.',steps:['Jednorázově použij počítač pro první nastavení.','Nainstaluj SideStore přes iloader podle oficiálního návodu.','Potom používej SideStore s LocalDevVPN přímo v zařízení.'],primary:'Otevřít instalaci SideStore ↗',url:'https://docs.sidestore.io/docs/installation/install',secondary:'Otevřít iloader ↗',secondaryUrl:'https://iloader.app/'},
    checkVersionNoPc:{title:'Nejdřív zjisti přesnou verzi iOS',text:'Cesta SideInstalleru bez PC aktuálně vyžaduje iOS/iPadOS 27. U ostatních verzí počítej s jednorázovým prvotním nastavením SideStore přes počítač.',steps:['Otevři Nastavení → Obecné → Informace.','Zjisti přesnou verzi iOS/iPadOS.','V průvodci vyber odpovídající verzi.'],primary:'Požadavky SideInstalleru ↗',url:'https://github.com/FrizzleM/SideInstaller',secondary:'Požadavky SideStore ↗',secondaryUrl:'https://docs.sidestore.io/docs/installation/prerequisites'},
  ...
  de: {
    ...
    pairingNeeded:{title:'iOS 17–26 ohne PC: für die Ersteinrichtung ist ein Computer nötig',text:'Die aktuellen offiziellen SideInstaller-Anforderungen nennen nur iOS/iPadOS 27. Unter iOS 17–26 verwende die normale SideStore-Erstinstallation mit einem Computer; danach können Installation und Refresh auf dem Gerät erfolgen.',steps:['Einmalig einen Computer für die Ersteinrichtung verwenden.','SideStore mit iloader nach der offiziellen Anleitung installieren.','Danach SideStore mit LocalDevVPN auf dem Gerät verwenden.'],primary:'SideStore-Installation öffnen ↗',url:'https://docs.sidestore.io/docs/installation/install',secondary:'iloader öffnen ↗',secondaryUrl:'https://iloader.app/'},
    checkVersionNoPc:{title:'Zuerst die genaue iOS-Version prüfen',text:'Der SideInstaller-Weg ohne PC erfordert derzeit iOS/iPadOS 27. Bei anderen Versionen ist eine einmalige SideStore-Ersteinrichtung am Computer einzuplanen.',steps:['Einstellungen → Allgemein → Info öffnen.','Genaue iOS/iPadOS-Version prüfen.','Danach die passende Version im Assistenten auswählen.'],primary:'SideInstaller-Anforderungen ↗',url:'https://github.com/FrizzleM/SideInstaller',secondary:'SideStore-Voraussetzungen ↗',secondaryUrl:'https://docs.sidestore.io/docs/installation/prerequisites'},
  ...
  es: {
    ...
    pairingNeeded:{title:'iOS 17–26 sin PC: hace falta un ordenador para la primera configuración',text:'Los requisitos oficiales actuales de SideInstaller indican únicamente iOS/iPadOS 27. En iOS 17–26 usa la instalación inicial estándar de SideStore con un ordenador; después puedes instalar y renovar desde el dispositivo.',steps:['Usa un ordenador una vez para la configuración inicial.','Instala SideStore con iloader siguiendo la guía oficial.','Después usa SideStore con LocalDevVPN en el dispositivo.'],primary:'Abrir instalación de SideStore ↗',url:'https://docs.sidestore.io/docs/installation/install',secondary:'Abrir iloader ↗',secondaryUrl:'https://iloader.app/'},
    checkVersionNoPc:{title:'Comprueba primero tu versión exacta de iOS',text:'La ruta de SideInstaller sin PC requiere actualmente iOS/iPadOS 27. En otras versiones, cuenta con una configuración inicial de SideStore usando un ordenador una vez.',steps:['Abre Ajustes → General → Información.','Comprueba la versión exacta de iOS/iPadOS.','Selecciona la versión correspondiente en el asistente.'],primary:'Requisitos de SideInstaller ↗',url:'https://github.com/FrizzleM/SideInstaller',secondary:'Requisitos de SideStore ↗',secondaryUrl:'https://docs.sidestore.io/docs/installation/prerequisites'},
  ...
  fr: {
    ...
    pairingNeeded:{title:'iOS 17–26 sans PC : un ordinateur est requis pour la première configuration',text:'Les exigences officielles actuelles de SideInstaller indiquent uniquement iOS/iPadOS 27. Sous iOS 17–26, utilisez l’installation initiale SideStore standard avec un ordinateur ; ensuite les installations et actualisations peuvent se faire sur l’appareil.',steps:['Utiliser un ordinateur une fois pour la configuration initiale.','Installer SideStore avec iloader en suivant le guide officiel.','Ensuite utiliser SideStore avec LocalDevVPN sur l’appareil.'],primary:'Ouvrir l’installation SideStore ↗',url:'https://docs.sidestore.io/docs/installation/install',secondary:'Ouvrir iloader ↗',secondaryUrl:'https://iloader.app/'},
    checkVersionNoPc:{title:'Vérifiez d’abord la version exacte d’iOS',text:'Le parcours SideInstaller sans PC nécessite actuellement iOS/iPadOS 27. Sur les autres versions, prévoyez une configuration initiale SideStore avec un ordinateur une seule fois.',steps:['Ouvrir Réglages → Général → Informations.','Vérifier la version exacte d’iOS/iPadOS.','Choisir la version correspondante dans cet assistant.'],primary:'Prérequis SideInstaller ↗',url:'https://github.com/FrizzleM/SideInstaller',secondary:'Prérequis SideStore ↗',secondaryUrl:'https://docs.sidestore.io/docs/installation/prerequisites'},
